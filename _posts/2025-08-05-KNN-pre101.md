...
In today's post, we'll explore KNN and Supervised Learning. We'll also discuss some applications in AI via a case study.
...
Some common **metrics** used to determine k-closet neighbors are listed in the table below. In the following formulae, $n$ is the number of features and $x_i$ and $y_i$ are coordinates of training and test points.
...
<table class = "language-python">
    ...
    <tr>
        <th scope='row'>Chebyshev distance</strong></th>
        <td>$$d = \lim_{p \to \infty} \left(\sum_{i=1}^n \|x_i - y_{i} \| ^p \right)^\frac{1}{p}$$</td>
    </tr>
    <tr>
        <th scope='row'>Euclidean  distance</strong></th>
        <td>$$d = \left(\sum_{i=1}^n \|x_i - y_{i} \| ^2 \right)^\frac{1}{2}$$</td>
    </tr>
    <tr>
        <th scope='row'>Manhattan distance</strong></th>
        <td>$$d = \sum_{i=1}^n \|x_i - y_{i} \|$$</td>
    </tr>
    <tr>
        <th scope='row'>Minkowski distance</strong></th>
...
## **KNN with K-D Tree**
The original KNN algorithm has its own drawback, so methods like K-D Tree are introduced to improve the algorithm's performance.
