...
In today's post, we'll explore classes and objects in **Object-Oriented Programming (OOP)**. We'll also discuss some applications in AI including using **PyTorch**. We aim to be able to read and explain the usage of classes in Pytorch later on.

<hr>

## **Introduction to OOP**
Like other parts of programming, OOP is developed to tackle real-world problems by organizing code into structures that mirror real-life situations.

### **Why OOP?** 
Previously, we learned about built-in data structures like integer, string, list, and dictionary. However, they are too basic to handle complicated reality which requires solutions to be customized concept by concept and even case by case. Classes and objects were introduced to address this need. A class can be considered a user-defined data type.

:question: It may seem functions can handle involved situations just fine. Why else do we need classes and objects? How are they better than functions? 

Let's say you want to learn how to cook. Applying procedural programming is to use some available recipe for a specific dish. If you wish to change an ingredient or exclude a spice, you have to ensure such substitutions keep adequate natural nutrition, taste, and texture of the rest. It might be easy to do so for only one, but imagine using a dozen of them, it would be a pain in the neck. On the other hand, from object-oriented programming viewpoint, learning first three or four fundamental cooking techniques, basic ingredients, seasonings, and measurements, you'd be able to create your own recipes. Moreover, you can practice others' recipes and modify it as much as you like and already know. 
...
We'll first look at some real-world examples. You can think of your own examples based on these if that help you understand the concepts better.
...
- Example 4: Animal can be considered a class in the context of a zoo where cats, dogs, and tigers are part of it. They are called objects of the class "Animal".
...
Essentially, a class is an enclosed generalization of many objects. What can be a class and what can be an object depends on a project's setting. There is no fixed solution for this matter. 

Keep in mind that attributes refers to properties of a class and are just variables but values since a class is like a template or an outline. Take the class "Cat" in example 3, "hair" can be its property and for each of its objects, either no hair, short hair, or long hair would be specified. 

Methods refers to behaviors or actions of a class. The class "Animal" can have walk, sit, sleep, and eat as methods that its objects all have.
...
Before putting the ideas into writing, it's imperative you convey them clearly to yourself and/or your collaborators. In terms of OOP, one standard step is to represent the structure of a class using class diagram rather than a regular drawing. 

If a class diagram is a house, it has two stories with a roof. The class name stays in the roof; its attributes stay on the second floor; its methods stay at ground level. 

~~DEMONSTRATION~~

There are three parts that should be displayed for each attribute and method. They are access modifiers, attribute/method names, and (returned) data structures from left to right. A colon is needed between the last two as you can see in the picture.

:question: You may wonder, "What are access modifiers?"

Great question! Earlier, we said a class is like an enclosed container. Take ChatGPT, its free version with which limited actions are allowed is open to the public and those subscribing to plus are exposed to more features but not all while pro members have access to even the latest release. We'll put down some new terms in a moment. You can skim over this part to get yourself familiar with those and come back later when they are carefully mentioned.
...
<br>
You might see that an outsider can use protected variables when trying it out, but it's best practice not to do so. 
...
- The **constructor** `__init__()` is to initialize attributes whenever an object is made. It's conventional to use this special method, but creating a class without it or placing attributes in a regular method is also accepted. Assigning an object is a bit different in these two cases.
...
- The **special parameter** `self` is mandatorily included first thing when a method is defined in Python. It, which is adopted in Python by convention, is not a keyword and can be replaced by any word (except a keyword). To understand what it does, think about visiting someone's house, you'd need to ask the owner if you can come in and if you can touch/use an item. Python employs `self` to refer to the owner that is a class containing it or an object of such a class. Anytime you want to use an attribute or a method, access them through `self` using a dot `.` just like methods in previous posts.

- An attribute's name is allowed to be the same as its parameter's to avoid wasting resources, but it's important to distinguish them when reading or writing code. 
...
A class' attributes are regularly set private, so only that class can employ them. To handle this issue, you can write two methods for each attribute: the getter is to touch the attribute and the setter is to assign it a value. These two methods allows you to read a private attribute and modify it as desired without producing any error.
```python
class Cat:
...
```
The child class inherits their parent class' regular methods via `ChildClassName(ParentClassName)` while it needs `super().__init__(parent_att_1, ..., parent_att_n)` to inherit the attributes particularly when the child class have their own other attributes.
...
- The latter is called a child, derived, extended, or child class. 

In a class diagram, this relationship is represented by an arrow with a hollow tip from the parent class to the child class. It can be interpreted that information in a parent class also belongs to their child class. 

Take examples 4 and 5 at the beginning of Section [Classes and Objects](#class-object), Animal can be the parent class while Cat is its child class.

### **Reusability** 
Since a child class shares most properties and methods of their parent class, they can use those as their own; those are either public or protected. However, a child class cannot use private ones, for example, children are not allowed to read anyone's diary including their parent class'. That's theory, but in practice, attributes are privated defined and getter methods are attached in a class.
...
If `super().__init__()` is replaced by `ParentClassName.__init__(self)`, there must be a `self` so that the program knows which parent class the child class takes initiations from.
...
Now a manager is also an employee of a company, but their salary is for sure computed differently from other workers.
```python
class Employee:
    def __init__(self, name, salary):
        self._name = name
...
Think of the Animal class, most of them make sounds but theirs are different from each other. So we want the Animal class to have the sound method and each child class to have one as well. To handle this situation, we need to install packages named `ABC` and `abstractmethod` as well as indicate these modules' usage via `ParentClassName(ABC)` and `@abstractmethod` right above the shared method and pass inside the method in the parent class. With this syntax utilized, all the parent and child classes must share such a method name. 
```python
